...
public class DatabaseManager
{
	...
	public Client getClient( String name )
	{
		try
		{
			PreparedStatement ps = connection.prepareStatement("SELECT * FROM client WHERE concat(fname,\" \",mname, \" \", lname)= ? OR concat(fname, \" \", lname) = ?");
			ps.setString(1, name);
			ps.setString(2, name);
			ResultSet  rs = ps.executeQuery();
	...
	public void updateClientLastVisit( Client c, Date date )
	{
		try
		{
			PreparedStatement ps = connection.prepareStatement("UPDATE client SET dateLastVisited = ? WHERE client_id = ?");
			ps.setDate(1, date);
	...
	public void editProduct(Product p, double price, String measurement, String type)
	{
	...
	public Iterator getAllServiceTransactionsOfReceipt( int receipt_id )
     {
         try
         {
             PreparedStatement ps = connection.prepareStatement("SELECT concat(C.fname, \" \",C.lname) as clientName, S.name as serviceName, E1.name as Senior, E2.name as Junior, S.price\n" +
	...
	public Iterator getAllProductTransactionsOfReceipt( int receipt_id )
     {
         try
         {
             PreparedStatement ps = connection.prepareStatement("SELECT concat(C.fname, \" \",C.lname) as clientName, P.name as productName, PLI.quantity, p.price, (PLI.quantity*P.price) as total  " +
                                                                 "FROM taylortyler.transaction T, taylortyler.receipt R, " +
                                                                 "taylortyler.transactionlist TL, taylortyler.client C, " +
	...
	private ArrayList<Integer> addProductLineItem(ArrayList<ProductLineItem> products)
	{
		try
		{
			PreparedStatement ps = connection.prepareStatement("INSERT INTO productlineitem(product_id, quantity, employee_id) "
					  + "VALUES (?, ?, ?)",Statement.RETURN_GENERATED_KEYS);
			...
			for(int i = 0; i < products.size(); i ++)
			{
				...
				ps.setInt(3, Integer.parseInt(products.get(i).getEmployee().getsEmployeeId()));
				ps.executeUpdate();
				ResultSet generatedKeys = ps.getGeneratedKeys();
				generatedKeys.next();
